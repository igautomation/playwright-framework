...
export function getProjectConfig(projectName = 'default', env = 'dev') {
  ...
  if (!config) {
    logger.error(`Project configuration not found for ${projectName}. Using default.`);
  ...
  if (missingVars.length > 0) {
    logger.warn(
      `Missing environment variables for ${projectName}: ${missingVars.join(', ')}. Using defaults.`
    );
  }
