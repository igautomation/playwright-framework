...
test.describe("XPath Practice Page UI Smoke Tests", () => {
  ...
  test("Submit form with invalid email and verify validation", async ({
    ...
    try {
      ...
      const emailError = await authenticatedPage.textContent(
        "#validation-message"
      );
      ...
      throw new Error(
        "XPath Practice Page form validation test failed: " + error.message
      );
    }
  });
  ...
  test("Fill all fields and submit the form successfully", async ({
    ...
    try {
      ...
      await retryDiagnostics(error);
      throw new Error(
        "XPath Practice Page valid submission failed: " + error.message
      );
    }
  });
});
