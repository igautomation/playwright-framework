...
test.describe('Create User API Tests', () => {
  ...
  test.beforeEach(({ request }) => {
    // Create a new API client with the correct URL from config
  ...
  test('Update user with PUT request', async ({ request }) => {
    ...
    const responseData = await apiClient.put(
      `${config.paths.reqres.users}/${config.api.testData.userId || 2}`,
      updatedUserData
    );
  ...
  test('Delete user', async ({ request }) => {
    try {
      ...
      await apiClient.delete(`${config.paths.reqres.users}/${config.api.testData.userId || 2}`);
