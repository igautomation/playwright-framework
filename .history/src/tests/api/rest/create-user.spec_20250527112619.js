...
test.describe('Create User API Tests', () => {
  ...
  test.beforeEach(({ request }) => {
    // Use direct URL to bypass config issues
  ...
  test('Update user with PUT request', async ({ request }) => {
    ...
    const updatedUserData = {
      ...
      job: config.api.testData.newUser?.job ? `Senior ${config.api.testData.newUser.job}` : 'Senior Engineer'
    };
    
    // Send PUT request to update a user
    const responseData = await apiClient.put(`/users/${config.api.testData.userId || 2}`, updatedUserData);
  ...
  test('Update user with PATCH request', async ({ request }) => {
    ...
    const partialUserData = {
      job: config.api.testData.newUser?.job ? `${config.api.testData.newUser.job} Manager` : 'Product Manager'
    };
    
    // Send PATCH request to update a user
    const responseData = await apiClient.patch(`/users/${config.api.testData.userId || 2}`, partialUserData);
