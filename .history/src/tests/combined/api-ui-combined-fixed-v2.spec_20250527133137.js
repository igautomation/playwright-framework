...
const { PlaywrightErrorHandler } = errorHandler;
...
const selectors = {
  ...
  employeeNameInput:
    process.env.EMPLOYEE_NAME_INPUT || config.selectors?.combined?.employeeNameInput,
...
test.describe('API-UI Combined Tests V2', () => {
  ...
  test('should create user via UI and verify via API', async ({ page, request }) => {
    ...
    await PlaywrightErrorHandler.withSelfHealing(
      async selector => {
        // Select User Role
  ...
  test('should update employee via API and verify changes in UI', async ({ page, request }) => {
    ...
    const updateData = await PlaywrightErrorHandler.withRetry(
      async () => {
