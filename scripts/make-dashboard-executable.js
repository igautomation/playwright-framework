...
const cliPath = path.resolve(__dirname, '../src/cli/dashboard.js');
...
try {
  ...
  const symlinkPath = path.join(binDir, 'dashboard');
...
try {
  ...
  packageJson.bin.dashboard = 'src/cli/dashboard.js';
  
  // Write updated package.json
  fs.writeFileSync(packageJsonPath, JSON.stringify(packageJson, null, 2));
  console.log('✅ Updated package.json bin section');
  
  // Install open package for browser opening
  console.log('📦 Installing open package...');
  execSync('npm install --save open', { stdio: 'inherit' });
